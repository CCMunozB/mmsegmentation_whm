...
model = dict(
    ...
    decode_head=dict(
        ...
        dict(type='DiceLoss', loss_name='loss_dice', loss_weight=0.2, class_weight=[0.0001, 1.0])
        ]),
    auxiliary_head=dict(
        in_channels=768, 
        num_classes=2,
        ...
        dict(type='DiceLoss', loss_name='loss_dice', loss_weight=0.05, class_weight=[0.0001, 1.0])
        ])
    )
