...
model = dict(
    ...
    decode_head=dict(
        ...
        dropout_ratio=0.3,
        loss_decode=[
        dict(type='LogCoshDiceLoss', loss_name='loss_cosh', loss_weight=1.0, #class_weight=[0.001, 1.0]
             )
        ]),
    # auxiliary_head=dict(
    #     in_channels=768, 
    ...
    #     num_convs=3,
    #     loss_decode=[
    #     dict(type='FocalTverskyLoss', loss_name='loss_ftl', loss_weight=0.6, class_weight=[0.001, 1.0]),
    #     dict(type='DiceLoss', loss_name='loss_dice', loss_weight=0.2, class_weight=[0.001, 1.0])
    #     ])
    )
