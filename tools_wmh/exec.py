...
with warnings.catch_warnings():
    ...
    model = init_model(f"work_dirs/remos_v3/remos_v3.py", f"work_dirs/remos_v3/iter_40000.pth", "cuda:0")
    #model = init_model(f"configs/remos/{model_name}.py", f"work_dirs/{model_name}/iter_40000.pth", "cuda:0")
...
for i in range(data.shape[0]):
    ...
    pre_print = np.moveaxis(result.cpu().numpy(), 0, 2)
    tifffile.imwrite('tools_wmh/output/test_images/{}.tiff'.format(num), slice_data)
    mmcv.imwrite(pre_print, 'tools_wmh/output/test_label/{}.png'.format(num))
...
final_segmentation = (final_segmentation >= 0.5)
#Save File 
