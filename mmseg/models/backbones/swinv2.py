...
class ShiftWindowMSA(BaseModule):
    def __init__(self,
        ...
        # Using the modified WindowMSA class with log-spaced bias
    ...
    def forward(self, query, hw_shape):
        ...
        query_windows = self.window_partition(shifted_query).view(-1, self.window_size**2, C)
...
class SwinTransformerV2(BaseModule):
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            ...
            #out_hw_shape = tuple(i * 1 for i in out_hw_shape)
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')
                out = norm_layer(out)
                out = out.view(-1, *out_hw_shape,
                               int(self.num_features[i]/1)).permute(0, 3, 1,
                                                             2).contiguous()
                outs.append(out)
