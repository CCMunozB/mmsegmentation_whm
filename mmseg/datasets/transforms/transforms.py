...
RANDAUG_SPACE = [[dict(type='mmdet.Rotate', img_border_value=0.0)], [dict(type='mmdet.ShearX', img_border_value=0.0)],
                 [dict(type='mmdet.ShearY', img_border_value=0.0)], [dict(type='mmdet.TranslateX', img_border_value=0.0)],
                 [dict(type='mmdet.TranslateY', img_border_value=0.0)]]
...
class ShearY(GeomTransform):
    ...
    def _transform_seg(self, results: dict, mag: float) -> None:
        ...
        for key in results.get('seg_fields', []):
            results[key] = mmcv.imshear(results[key],
                mag,
                direction='vertical',
...
class TranslateY(GeomTransform):
    ...
    def _transform_seg(self, results: dict, mag: float) -> None:
        """Translate segmentation map vertically & horizontal."""
        mag = int(results['img_shape'][0] * mag)
        for key in results.get('seg_fields', []):
            results[key] = mmcv.imtranslate(results[key],
                mag,
                direction='vertical',
                border_value=self.seg_ignore_label,
                interpolation='nearest')
...
@TRANSFORMS.register_module()
class RandomMultiRotFlip(BaseTransform):
    """Rotate and flip the image & seg or just rotate the image & seg.
    ...
    def random_rot_flip(self, results: dict) -> dict:
        ...
        for key in results.get('seg_fields', []):
            results[key] = np.rot90(results[key], k)
        for key in results.get('seg_fields2', []):
            results[key] = np.rot90(results[key], k)
